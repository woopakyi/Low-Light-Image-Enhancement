...
# ==============================
# Apply Blurry Background (Bokeh) Effect
# ==============================
def apply_bokeh(enhanced_np, subject_mask):
    ...
    # Composite: sharp subject + dreamy blurred background
    mask_3d = np.stack([bokeh_mask] * 3, axis=2)
    bokeh_result = enhanced_np * mask_3d + blurred_bg * (1 - mask_3d)

    # Slightly enhance subject for pop
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Apply blurry background (bokeh) effect to low-light image using mask")
    ...
    bokeh_result = apply_bokeh(enhanced_np, subject_mask)

    # Visualize
    plt.figure(figsize=(12, 10))
    plt.imshow(bokeh_result)
    plt.title(f"Final Bokeh Result: {filename}\n"
